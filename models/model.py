...
class TransactionModel(Base):
    ...
    providerStatus = Column(String(25), nullable=True)
    providerReference = Column(String(150), nullable=True)
    providerDescription = Column(String(250), nullable=True)
    amount = Column(String(50), nullable=False)
    statusCode = Column(String(5), nullable=True, default="C001",server_default="C001")
    statusMessage = Column(String(50), nullable=True, default="Processing",server_default="Processing")
    debitStatus = Column(String(5), nullable=True )
    debitReference = Column(String(150), nullable=True)
    debit_description = Column(String(250), nullable=True)
    token= Column(String(255), nullable=True)
    resetToken= Column(String(255), nullable=True)
    configureToken= Column(String(255), nullable=True)
    unit = Column(String(255), nullable=True)
    recipient = Column(String(255), nullable=True)
    unitType = Column(String(255), nullable=True)
    customerName = Column(String(255), nullable=True)
    customerAddress = Column(String(255), nullable=True)
    product = relationship("ProductModel",  back_populates="transactions")
    product_type = relationship("ProductTypeModel", back_populates="transactions")
    ...
    provider = relationship("ServiceProviderModel", back_populates="transactions")
    updated_at = Column(DateTime,server_default=func.now(),server_onupdate=func.now())
    created_at = Column(DateTime,server_default=func.now())
class OTPModel(Base):
    __tablename__ = "otps"
...
class ProductTypeModel(Base):
    ...
    transactions = relationship("TransactionModel", back_populates="product_type")
    updated_at = Column(DateTime,server_default=func.now(),server_onupdate=func.now())
    created_at = Column(DateTime,server_default=func.now())
...
class ServiceProviderModel(Base):
    ...
    transactions = relationship("TransactionModel", back_populates="provider")
    updated_at = Column(DateTime,server_default=func.now(),server_onupdate=func.now())
    created_at = Column(DateTime,server_default=func.now())